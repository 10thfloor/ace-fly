...
describe('Security Construct', () => {
  ...
  it('should handle errors when adding a secret', async () => {
    (flyctlExecute as any).mockRejectedValue(new Error('Failed to add secret'));
  ...
  it('should list secrets successfully', async () => {
    ...
    (flyctlExecute as any).mockResolvedValue({ stdout: mockSecrets, stderr: '' });
  ...
  it('should handle errors when enforcing HTTPS', async () => {
    (flyctlExecute as any).mockRejectedValue(new Error('Failed to enforce HTTPS'));
